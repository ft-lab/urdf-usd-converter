...
class TestConverterPhysics(ConverterTestCase):
    ...
    def test_physics_link_box(self):
        ...
        self.assertTrue(Gf.IsClose(mass_api.GetDiagonalInertiaAttr().Get(), Gf.Vec3f(100, 100, 100), self.tolerance))
        self.assertTrue(Gf.IsClose(mass_api.GetMassAttr().Get(), 0.8, self.tolerance))
        self.assert_rotation_almost_equal(Gf.Rotation(mass_api.GetPrincipalAxesAttr().Get()), Gf.Rotation(Gf.Quatf(1, Gf.Vec3f(0, 0, 0))), 1e-4)
