...
__all__ = ["resolve_ros2_package_paths"]


def resolve_ros2_package_paths(uri: str, data: ConversionData) -> str:
    """
    Resolve the ROS2 package paths for the given filename.
    ...
    if uri.startswith("package://"):
        ...
        if len(split_dirs) < 2:
            raise ValueError(f"Invalid ROS2 package URI. No relative path specified: {uri}")
        package_name = split_dirs[0]
        package_path = data.ros2_packages.get(package_name, None)
        if package_path:
